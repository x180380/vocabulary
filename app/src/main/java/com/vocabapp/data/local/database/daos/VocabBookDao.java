...
public interface VocabBookDao {
    ...
    @Query("SELECT v.*, COUNT(bw.id) AS word_count FROM vocab_books v LEFT JOIN book_words bw ON v.book_id = bw.book_id WHERE v.is_mine = 0 GROUP BY v.book_id ORDER BY v.book_id ASC")
    LiveData<List<VocabBookWithCount>> getLibraryBooksWithCount();
